...
def add_action(con, action, action_code, user_id):
    """
    add action to actions table in database
    :param con: connection to database
    :param action: action name
    :param action_code: code of action
    :param user_id: which user has done certain action
...
def generate_report(con, project_path):
    """
    generating daily reports if there is actions done for certain date
    :param con: database connection
    :param project_path: path of this project
            ...
            for key, action in action_summary.items():
                if key == "actions":
                    for act_key, act in action.items():
                        action_content += f"Action {act_key.upper()} is done: {act} times.\n"
